...
| `process_sync_batch` | `p_device_id uuid, p_operations jsonb` | jsonb | Offline-sync gateway (push side). Each operation in the array may carry `domain_operation` (P3.7, 2026-08-28) — the fine-grained dispatch key (`ticket.create`, `ticket.item_update`, etc., AD-021's allowlist); omitting it means the operation is recorded but never applied (`REJECTED unsupported_operation_type`). See §6 and `SCHEMA-REFERENCE.md` §12. |
| `sync_pull` | `p_device_id uuid, p_cursor bigint default 0, p_page_size int default 200` | jsonb | P3.7 (2026-08-28), the pull side — previously did not exist at all. Returns `{changes, next_cursor, has_more}`, cursor-paginated over `sync_changes.sequence_id`. `SECURITY INVOKER`: authorization is inherited from `sync_validate_device()` (device ownership/revocation) and the existing `sync_changes_select` RLS policy (tenant/branch/live-device), not reimplemented. |
