...
| `get_sales_breakdown` | `p_branch_id uuid, p_period text default 'today', p_start date, p_end date` | jsonb | **P9.9 Q4 / AD-027.** `{scope, totals, by_method[5], by_staff, recent[≤30]}`. Scope `full` (owner/admin/the branch's manager), `branch` (supervisor/accountant: `by_staff` null, seller names hidden), `own` (cashier/driver/other manager: only rows they created/recorded). Collections by `payments.method`, refunds attributed to the refunded payment's method. Refused without branch access or for other roles. |
