...
| `create_organization_invite` | `p_email text, p_role_key text, p_branch_id uuid DEFAULT NULL, p_valid_days integer DEFAULT 7, p_phone text DEFAULT NULL` | jsonb | **AD-026:** by role, to exactly one of `p_email` / `p_phone` (E.164; separators stripped). Callers: owner (any role), admin (below admin), branch manager (cashier/baker/driver/supervisor into a branch they manage — `private.manages_branch`). Refusals carry `invalid_request` reasons `contact_required`/`invalid_email`/`invalid_phone`/`role_required`, or `insufficient_role` (reason `not_your_branch`). Sweeps lapsed pending invites to `expired` (AD-025). Mints a raw token, stores only its hash, sets expiry. Delivery is the `send-invite-email` Edge Function, deployed and live-verified 2026-08-22 — see §7 and `BLOCKERS.md` §BLOCKER-001. |
| `accept_organization_invite` | `p_raw_token text` | jsonb | Hashes and matches token. **AD-025/026:** the caller's confirmed email (email invite) or SMS-confirmed phone (phone invite) must match, else `insufficient_role` reason `email_mismatch`/`phone_mismatch`. An expired invite is marked `expired` and returns `{accepted:false, code:'invalid_transition', status:'expired'}`. Otherwise creates `user_roles` and `branch_assignments`, marks invite accepted, returns `accepted:true`. |
| `revoke_organization_invite` | `p_invite_id uuid` | jsonb `{invite}` | **P9.9 Q7 (2026-09-17).** Pending or expired → `revoked`; audited `status_change`. Callers: whoever may create that invite (owner any; admin below admin; branch manager crew invites for a branch they manage — `private.can_manage_invite`). Unknown id and not-yours both `insufficient_role`; accepted/revoked `invalid_transition`. `token_hash` never returned. |
| `resend_organization_invite` | `p_invite_id uuid, p_valid_days integer default 7` | jsonb `{invite, raw_token}` | **P9.9 Q7.** Pending or expired → `pending` with a NEW token hash and fresh expiry (the old link stops working); audited `update` with `resent: true`; counts against `org_invite_create` rate limit (20/hour). Same callers and refusals as revoke; a second pending invite for the same contact raises `unique_violation` (`duplicate_reference`). Email invites are then handed to `send-invite-email` by the client. |
| `search_workspace` | `p_query text, p_limit integer default 6, p_only_my_orders boolean default false` | jsonb `{query, customers[], orders[], products[]}` | **P9.9 Q6.** **SECURITY INVOKER** — the caller's own RLS decides visibility. Case-insensitive substring match with LIKE wildcards escaped; customers by name or phone digits (0 / 234 prefixes equivalent, 5+ digits), orders by `ticket_number` or customer name (optionally only the caller's own), products by name / variant name / SKU with `variant_count` and `price_from` (exact text). Starts-with first; < 2 characters returns empty groups; `p_limit` 1–20. |
| `update_my_profile` | `p_full_name text, p_phone text default null` | jsonb `{id, full_name, phone, avatar_url}` | **P9.9 Q8.** Own row only. Name trimmed, 1–120 chars (`invalid_name`); phone E.164 after stripping separators, or empty to clear (`invalid_phone`). Audited (`profile` / `update`) when an organization is known. The contact phone is not the sign-in phone. Used instead of direct `profiles` UPDATE (see TECHNICAL_DEBT TD-019). |
...
| `complete_ticket` | `p_order_id uuid, p_warehouse_id uuid` | jsonb | Writes the sale stock movement — which is why it takes a warehouse and must be an RPC. Reachable now that the full lifecycle is (`STATE-MACHINES.md` §1); its own `reference_type` bug (found separately) was fixed 2026-08-22, see `BLOCKERS.md`. **Idempotency fixed 2026-09-01** (found writing `tests/sql/sales_write_rls.sql`): a second call on an already-`completed` ticket used to silently re-sell the same stock (its status UPDATE was a same-status no-op, so nothing ever raised to stop the movement loop above it running twice). Now rejects outright — `P0001`, `invalid_transition`/`already_completed` — if the ticket is already completed. |
...
| `archive_catalog_entity` | `p_entity_type text, p_entity_id uuid` | jsonb | 2026-09-01, resolving `BLOCKERS.md` BLOCKER-010(c). `p_entity_type` is `'product'\|'product_category'\|'product_variant'` — **not** `'ingredient'`/`'recipe'`, deliberately (AD-022 revoked `authenticated`'s ingredient-table grants; this `SECURITY DEFINER` RPC must not reopen them). Exists because a direct PostgREST `UPDATE` can never set `deleted_at` on these tables for any role — Postgres refuses a write that would make the new row fail the table's own SELECT policy (`deleted_at IS NULL`), confirmed live including with an `ALTER POLICY` probe that ruled out a role-gate explanation. Requires `has_permission('products.manage', NULL)` (owner/admin/branch_manager). Rejects (`P0001`) not-found/cross-tenant/already-archived alike, without distinguishing which. |
| `restore_catalog_entity` | `p_entity_type text, p_entity_id uuid` | jsonb | Symmetric to `archive_catalog_entity` (same migration, same day, same role gate, same entity-type scope) — clears `deleted_at`/`deleted_by`. `docs/SOFT-DELETE-AND-RETENTION.md` §38 specified this one first; building `archive_catalog_entity` alongside it was the new finding. |
...
| `set_supervisor_permission_override` | `p_profile_id uuid, p_permission_key text, p_granted boolean` | jsonb | Added 2026-09-02, `BLOCKERS.md` BLOCKER-025 (RESOLVED). The per-Supervisor, manager-configurable permission toggle `customer.update`'s resolution (BLOCKER-024) had asked for but that had no backing schema. Caller must hold `branch_manager` in the tenant (literal, deliberate — not owner/admin/self, per the owner's decision); `p_profile_id` must currently hold `supervisor` in that same tenant, or `P0001 not_a_supervisor`. `p_permission_key` is checked against a fixed, safe allowlist of Supervisor-relevant keys (`branch.view`, `customers.create/update/delete`, `financial.audit.confirm/submit`, `financial.expense.create/update`, `financial.view`, `reports.view`, `staff.view`, `tickets.create/view/correct/archive`) — anything else, including `tickets.update`/`tickets.cancel` (never grantable to any role, see `docs/ROLES-AND-PERMISSIONS.md`), is rejected `42501 permission_not_overridable`. `p_granted=true` raises, `false` lowers, `NULL` clears the override and reverts to the role default. Table `user_permission_overrides` has no direct `authenticated` write grant — this RPC is the only write path. `has_permission()` checks an active override first (wins outright over the role-level grant) — see `docs/ROLES-AND-PERMISSIONS.md`. |
| `process_sync_batch` | `p_device_id uuid, p_operations jsonb` | jsonb | Offline-sync gateway (push side). Each operation in the array may carry `domain_operation` (P3.7, 2026-08-28) — the fine-grained dispatch key (`ticket.create`, `ticket.item_update`, `customer.create`, `customer.update`, etc., AD-021's allowlist); omitting it means the operation is recorded but never applied (`REJECTED unsupported_operation_type`). May also carry `client_sequence` (P3.7, 2026-08-29) — captured verbatim, purely diagnostic, never enforced (OFFLINE-SYNC-MODEL.md §16). **Response shape corrected 2026-08-29**: each `results[]` entry now reflects the operation's actual post-dispatch outcome — `status` (`APPLIED/REJECTED/CONFLICT`), `error_code`, `result` — not a pre-dispatch snapshot (previously every operation reported `PENDING`/`CONFLICT` regardless of what actually happened). A replay (same `operation_id`, identical immutable context including `payload`) returns `replayed:true` plus the original `status`/`error_code`/`result`; a replay with an altered payload/`base_revision`/`branch_id`/`entity_type`/`domain_operation`/`tenant_id` is rejected outright (`42501`), not silently accepted. See §6 and `SCHEMA-REFERENCE.md` §12. **`customer.create` payload** (P3.7, 2026-08-29): `{full_name (required, 1-200 chars), phone?, email?, address_line?, notes?, is_walk_in?}` — result `{customer_id, full_name, revision:1}`. **`customer.update` payload**: `{customer_id (required, must equal the operation's own entity_id), full_name (required — full-value replacement, not a merge), phone?, email?, address_line?, notes?, is_walk_in?}` — result `{customer_id, full_name, revision}`. `customer.create` requires owner/admin/branch_manager/supervisor/cashier, or driver (`docs/ROLES-AND-PERMISSIONS.md`). `customer.update` is narrower, per an explicit product decision 2026-08-29 (`BLOCKERS.md` BLOCKER-024, resolved): owner, admin, or branch_manager always; supervisor only while holding the supervisor role in that tenant (today's coarse, role-level toggle — a finer per-supervisor manager-configurable grant was requested but has no backing schema yet, see `BLOCKERS.md` BLOCKER-025); driver and cashier cannot call `customer.update` at all, despite both holding `customer.create`. `customers` has no `branch_id` of its own — the operation's `branch_id` is optional/informational and, if supplied, is still checked by the existing generic branch-authorization gate. **`inventory.adjust`/`inventory.waste` payload** (P3.7, 2026-08-30): `{warehouse_id (required), item_type: 'ingredient'|'product' (required), item_id (required), quantity_delta (required, numeric, nonzero — signed, not an absolute target unlike `adjust_stock`), note?}` — result `{movement_id, quantity_on_hand, previous, revision:1}`. `inventory.adjust` writes `stock_movements.reason='adjustment'` and requires owner/admin/branch_manager; `inventory.waste` writes `reason='waste'` (rejects `quantity_delta >= 0`, `22023`) and requires owner/admin/branch_manager/baker — both role sets mirror the live `adjust_stock` RPC verbatim. Either is rejected (`P0001`, `negative_stock_rejected`) if applying the delta would leave on-hand negative; the operation's `warehouse_id` must belong to the operation's own `branch_id` (`22023` otherwise). **`inventory.receive`/`.transfer`/`.consume` are no longer in the `domain_operation` allowlist at all (2026-08-31)** — decided out of MVP scope entirely (`BLOCKERS.md` BLOCKER-026, RESOLVED); a client submitting any of the three now gets a hard `23514 check_violation` that aborts the whole `process_sync_batch()` call, not a graceful per-operation `REJECTED`. **`production.start`/`production.cancel` payload** (P3.7, 2026-08-30): `{batch_id (required)}` — result `{batch_id, status, revision, started_at?}`. Both perform a plain guard-validated status update (`scheduled`→`in_progress`/`cancelled`); either is rejected (`P0001`, `invalid_transition`) if the batch isn't currently `scheduled`, or (`22023`) if its `branch_id` doesn't match the operation's own authorized branch. `production.start` requires owner/admin/branch_manager/baker; `production.cancel` requires owner/admin/branch_manager (no baker) — both mirror `guard_production_batch_transition()`'s own actor lists verbatim. **`production.record_output`/`production.record_waste` payload** (P3.7, 2026-08-31, `BLOCKERS.md` BLOCKER-027 RESOLVED): `.record_output` — `{batch_id (required), actual_quantity (required, numeric > 0), ingredient_actuals? (array of {ingredient_id, actual_quantity?, waste_quantity?}), warehouse_id?}` — result `{batch_id, result: <complete_production_batch()'s own {batch, movements}>, revision}`. `.record_waste` — `{batch_id (required), reason (required), ingredient_actuals?, warehouse_id?}` — result shape mirrors, wrapping `fail_production_batch()`'s own `{batch, movements}`. Both are thin sync-facing wrappers: confirmed live that `complete_production_batch()`/`fail_production_batch()` already take per-ingredient actual/waste quantities in ONE call and `production_batches` has only a single `actual_quantity`/`completed_at` — no schema support for multiple partial output/waste events per batch, so these ARE those two RPCs, not new events. Both require owner/admin/branch_manager/baker (mirrors `guard_production_batch_transition()`'s 'completed'/'failed' actors verbatim, identical to 'in_progress'); rejected (`P0001`, `invalid_transition`) if the batch isn't `in_progress`, or (`22023`) if its `branch_id` doesn't match the operation's own authorized branch. Both delegate to `complete_production_batch()`/`fail_production_batch()` with an explicit `p_tenant_id` (added 2026-08-31, additive/backward-compatible — previously these RPCs resolved tenant via the session's active org, which for a cross-org sync operation caused a false "batch not found" for a legitimately authorized actor). Revision continues `production.start`'s shared-entity-id ledger on the same batch. **`production.complete` is no longer in the `domain_operation` allowlist (2026-08-31)** — redundant with `.record_output`, removed the same way as the inventory three above; submitting it now aborts the whole batch call with `23514`. **`payment.create` payload** (P3.7, 2026-08-30): `{ticket_id (required), amount (required, numeric > 0), method (required: cash/card/transfer/pos), reference?, cash_session_id?, driver_trip_id?}` — result `{payment_id, ticket_id, amount, revision:1}`. Mirrors the live `record_payment()` RPC in full (ticket/branch/status checks, the AD-018 driver-trip cash-custody branch, the cash-till-session branch, invoice resolution), tenant-scoped via `has_role_in()` instead of `record_payment()`'s session-based `has_role()`. Requires owner/admin/branch_manager/cashier, or driver (no `financial.payment.*` key exists in the permissions catalog, so the RPC's own role array is the rule to mirror, unlike `customer.create`). Rejects (`P0001`) a cancelled ticket, an overpayment beyond the ticket's outstanding balance, a cash payment with no open till session, or a driver-trip payment where the trip isn't `in_transit` or isn't linked to the ticket; the till session and driver trip cannot both be supplied (`22023`). Insert triggers (`guard_payment_relationships`, `apply_payment_to_ticket`) derive `tickets.amount_paid`/`invoices.status` automatically. **`payment.reverse` payload**: `{payment_id (required), amount (required, numeric > 0), reason (required, 1-1000 chars)}` — result `{refund_id, payment_id, amount, revision}`, `revision` continuing the same payment's `sync_changes` lifecycle ledger (create=1, each reversal increments). Mirrors `record_refund()` in full; requires owner/admin/branch_manager. Rejects (`P0001`) a refund exceeding the payment's remaining balance. Both `payment.create`/`.reverse` write `operation_type='EVENT'` (payments/refunds are append-only — `prevent_financial_mutation()` blocks any UPDATE/DELETE). **`expense.create` payload**: `{category (required: ingredients/rent/utilities/salaries/transport/other), amount (required, numeric > 0), description?, paid_method? (cash/card/transfer/pos), cash_session_id? (required if paid_method='cash'), incurred_at?, receipt_url?}` — result `{expense_id, category, amount, revision:1}`, `operation_type='CREATE'` (expenses are mutable — no immutability trigger, direct edits permitted via `expenses_update` RLS). No RPC precedent exists; mirrors the live `expenses_insert` RLS role array (owner/admin/branch_manager/cashier) verbatim, which disagrees with the `role_permissions` catalog's `financial.expense.create` grants on `cashier` vs `supervisor` — an unresolved, logged discrepancy (`IMPLEMENTATION_LOG.md` 2026-08-30), not a guess. `expense.reverse` remains allowlisted but has no handler yet — no reversal mechanism exists anywhere in the live schema for expenses (`BLOCKERS.md` BLOCKER-028) — `REJECTED unsupported_operation_type`. |
...
| `get_revenue_report` | `p_branch_id uuid, p_period text default null, p_start date default null, p_end date default null` | jsonb | **P9.9 Q1 (2026-09-14).** Revenue and cash for one branch over a period: `{branch_id, period, start_date, end_date, timezone, day_count, totals, days[]}`. `p_period` ∈ `today`/`7d`/`30d`/`90d`/`month`/`last_month`, resolved in the organization's timezone; or explicit inclusive `p_start`/`p_end` (not both; at most 366 days). Each day row equals that day's `get_daily_revenue_summary()` plus `completed_tickets`; totals summed server-side; money as `numeric(19,4)::text`. Same callers as the daily summary (branch access + owner/admin/branch_manager/cashier). Refusals: `invalid_request` reasons `invalid_period`/`invalid_range`/`range_too_long`, `insufficient_role`. |
| `get_product_performance` | `p_branch_id uuid, p_period text default null, p_start date default null, p_end date default null, p_order text default 'value', p_limit integer default 100` | jsonb | **P9.9 Q2 (2026-09-14).** Per product variant sold in completed tickets over the period: `rank`, names, `units` (exact text), `line_value` (quantity × unit price, **before order-level discount/tax**), `orders`, `value_share_pct` (`NN.NN`, server-computed). Ranked by `value` or `units`; `p_limit` 1–200; `products_sold` counts all. Refunds are not attributed to products; no cost/margin (AD-022). Same callers and period rules as `get_revenue_report`. |
| `get_sales_breakdown` | `p_branch_id uuid, p_period text default 'today', p_start date, p_end date` | jsonb | **P9.9 Q4 / AD-027.** `{scope, totals, by_method[5], by_staff, recent[≤30]}`. Scope `full` (owner/admin/the branch's manager), `branch` (supervisor: `by_staff` null, seller names hidden), `own` (cashier/driver/other manager: only rows they created/recorded). Collections by `payments.method`, refunds attributed to the refunded payment's method. Refused without branch access or for other roles. |
| `get_branch_performance` | `p_period text default 'today', p_start date, p_end date` | jsonb | **P9.9 Q3 / AD-027.** Owner/admin: every branch (`scope: all`); branch manager: managed branches (`managed`); others refused. Per branch revenue, refunds, net collected, orders, staff, share of net revenue, 7-day net revenue trend. |
| `set_my_avatar` | `p_object_path text` | jsonb profile | **P9.9 Q9.** Path must be `<active org>/profiles/<uid>/<name>.(jpg|jpeg|png|webp)` and exist in `avatars` uploaded by the caller; NULL clears. Audited. |
| `mark_notifications_read` | `p_ids uuid[] default null` | integer | **P9.9 Q5.** Own unread notifications in the active organization (all when null). |
| `register_push_token` / `unregister_push_token` | `p_token text, p_platform text` / `p_token text` | jsonb / integer | **P9.9 Q5.** Expo push token per (organization, person); registering a token revokes it for anyone else. |
| `claim_push_batch` / `complete_push_batch` | `p_limit` / `p_results jsonb` | jsonb / integer | **service_role only**, used by Edge Function `dispatch-push`. |
