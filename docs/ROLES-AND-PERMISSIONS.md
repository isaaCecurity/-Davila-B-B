...
A Branch Manager can raise, lower, or fully reconfigure this for any individual Supervisor at any time. **Built 2026-09-02** (`BLOCKERS.md` BLOCKER-025, resolved by owner decision): `user_permission_overrides` (`tenant_id, profile_id, permission_id, granted`), written only via `set_supervisor_permission_override(p_profile_id, p_permission_key, p_granted)` — Branch Manager only (not Owner, not Admin, not the Supervisor themselves — a literal, deliberately narrow decision), target must currently hold `supervisor` in the same tenant, and only a fixed, safe allowlist of Supervisor-relevant keys is overridable: `branch.view`, `customers.create/update/delete`, `financial.audit.confirm/submit`, `financial.expense.create/update`, `financial.view`, `reports.view`, `staff.view`, `tickets.create/view/correct/archive`. `tickets.update`/`tickets.cancel` are deliberately excluded even though a loose "tickets.*" reading would include them — see the note above: those two must never be granted to any role, by any mechanism. `p_granted=true` raises, `false` lowers, `NULL` clears the override (reverts to the role default from `role_permissions`). `has_permission()` checks for an active override first and, if one exists, it wins outright over the role-level grant. See `docs/API-CONTRACT.md`'s `set_supervisor_permission_override` row and `tests/sql/supervisor_permission_overrides.sql` (17/17, live-verified).
