...
-- p_warehouse_id was trusted blindly when a caller supplied it. This overload is not directly
-- callable by authenticated/anon (only postgres/service_role) -- the real caller is
-- apply_production_record_output/apply_production_record_waste's 5-arg sibling
-- (see the next migration), currently dead per AD-022's domain_operation CHECK, but this
-- overload gets the same fix for consistency and to protect any future direct/service-role
-- caller.
