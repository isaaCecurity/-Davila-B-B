...
| P9.3 | Ticket creation (driver) | P4.4 | queued, immutable | Backend COMPLETE as of 2026-08-24 — ADR-001 Phases 2+3 (schema + RPC/security layer) are both live: `driver_trips`, full lifecycle RPCs (`start_driver_trip`/`verify_trip_loading`/`depart_driver_trip`/`return_driver_trip`/`reconcile_driver_trip`/`complete_driver_trip`), trip-scoped cash custody in `record_payment()`/`close_cash_session()` (AD-018), and the ticket-trip assignment guard. Driver-created tickets use the existing plain-INSERT ticket pattern (no `create_ticket()` RPC exists for anyone), now with `driver_trip_id` integrity enforced. Still BLOCKED on **BLOCKER-006** for the *offline* half specifically (queued/sync behavior) — the online RPC surface has no dependency on it and is fully usable. Frontend (Phase 5 of ADR-001, driver mobile UI) not started. |
