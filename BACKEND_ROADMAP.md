...
| P5.8 | Reporting & P&L | `20260810120000_reporting_views.sql` | **NOT_STARTED — no views/RPCs exist live** (`pg_views`/`pg_matviews` both empty; the migration file was never applied). `docs/REPORTING-MODEL.md` is thoroughly decision-locked (§85) and consistent with AD-017, and its two hard schema prerequisites — `organizations.timezone` and `stock_movements.unit_cost` — both already exist as columns. But `unit_cost` is **100% NULL on every row**, including `purchase`-reason movements, so weighted-average COGS (the locked costing method) cannot actually be computed from live data. **BLOCKER-018**, new 2026-08-24: how ingredient purchase cost gets captured is an unspecified product decision, not something to guess. The revenue/cash half of P5.8 (no costing dependency) is unblocked and buildable independently. |
...
| P9.3 | Ticket creation (driver) | P4.4 | queued, immutable | **Ticket creation ONLINE-COMPLETE 2026-08-25** — `apps/mobile/app/driver/sell.tsx` builds a cart from the catalog and creates a `draft` roadside ticket via `createRoadsideTicket()` (plain INSERT, RLS-verified live), then records a payment via the already-live `recordDriverTripPayment`. Still BLOCKED on **BLOCKER-006** for the *offline* half (queued/sync behavior) — unchanged. **New finding, BLOCKER-021**: the ticket cannot legally advance past `draft` — `guard_ticket_status_transition()`'s actor lists exclude `driver` at every hop, and the eight-status chain has no shortcut for stock that's already loaded. Sell therefore stops at `draft` + payment recorded; no confirm/complete exists here until BLOCKER-021 is resolved. See `BLOCKERS.md` §BLOCKER-021, `IMPLEMENTATION_LOG.md` 2026-08-25. |
