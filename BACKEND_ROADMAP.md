...
| P9.3 | Ticket creation (driver) | P4.4 | queued, immutable | BLOCKED (006 only — 019/020 RESOLVED 2026-08-24, see AD-018/AD-019). ADR-001 Phase 2 (database design) is COMPLETE and live: `driver_trips` table, `tickets.driver_trip_id`, `payments.driver_trip_id`, extended `stock_movements` reference_type — see `IMPLEMENTATION_LOG.md`. Phase 3 (RPC/security layer: trip lifecycle RPCs, transition guard, driver-trip payment recording, the `close_cash_session()` change AD-018 needs) not started. Driver-created tickets remain a trip-scoped workflow per ADR-001, not a standalone P9.3 slice. |
