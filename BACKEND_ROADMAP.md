...
| P9.3 | Ticket creation (driver) | P4.4 | queued, immutable | Backend COMPLETE as of 2026-08-24 — ADR-001 Phases 2+3+4 all done: schema, RPC/security layer, and `docs/STATE-MACHINES.md` §6 (Driver Trip) all live and cross-checked against each other. Driver-created tickets use the existing plain-INSERT ticket pattern (no `create_ticket()` RPC exists for anyone), now with `driver_trip_id` integrity enforced. Still BLOCKED on **BLOCKER-006** for the *offline* half specifically (queued/sync behavior) — the online RPC surface has no dependency on it and is fully usable. **Frontend: ADR-001 Phase 5 first slice live 2026-08-25** — `apps/mobile/app/driver/home.tsx` + full `driver_trips` data layer, covering Start Trip/Go/Return. Sell (ticket creation) not yet wired — no driver-facing product/customer selection screen exists yet. |
