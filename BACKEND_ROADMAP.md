...
| P9.4 | Inventory view & adjust | P4.2 | online-only for now — queuing needs P10 | **READ PATH COMPLETE / WRITE PATH COMPLETE 2026-08-21. Narrowed to finished-product stock only 2026-09-01 (AD-022) — ingredient tracking deactivated for MVP.** `AdjustStockAction` on each stock row calls the already-existing `adjust_stock()` RPC (P4.2b) — an absolute target, not a delta; three reasons (`adjustment`, `waste`, `opening_balance`), role-gated per reason server-side. `adjust_stock()` now rejects `item_type='ingredient'` outright; `app/inventory/[warehouseId].tsx`'s ingredient/product tab toggle was removed, the screen always shows product stock. |
| P9.5 | Production batches | P4.3 | online-only for now — queuing needs P10 | **READ PATH COMPLETE 2026-08-16 / WRITE PATH COMPLETE 2026-08-21. Deactivated for MVP 2026-09-01 (AD-022)** — production batches are entirely recipe/ingredient machinery, out of scope alongside ingredient tracking; the "Batches" nav button was removed from the catalog screen, and `complete_production_batch()`/`fail_production_batch()` lost their `authenticated` EXECUTE grant. Screen/RPCs untouched in the repo for v2. Historical detail (BLOCKER-017, the trigger-side guard flag) unchanged below. |
